package com.evport.businessapp.utils.loader
...
class Glide4Engine : ImageEngine {
    ...
    //单例
    private var instance: Glide4Engine? = null

    //单例模式，私有构造方法
    private fun GlideEngine() {}

    //获取单例
    fun getInstance(): Glide4Engine? {
        if (null == instance) {
            synchronized(Glide4Engine::class.java) {
                if (null == instance) {
                    instance = Glide4Engine()
                }
            }
        }
        return instance
